...
export default function EmpresaEditarClient({ empresa }: Props) {
  ...
  return (
    <div className="max-w-5xl space-y-8 animate-in fade-in duration-700">
      ...
      <Form action={formAction} className="bg-card rounded-[2rem] border border-border p-8 shadow-sm space-y-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            <input name="estado" defaultValue={empresa.estado} required maxLength={2} className="w-full px-4 py-2.5 rounded-xl border border-border bg-card focus:ring-2 focus:ring-accent outline-none font-bold text-center uppercase shadow-sm text-sm" />
          </div>
        </div>
        ...
        <div className="pt-4 flex justify-end">
          <button
            type="submit"
            disabled={pending}
            className="px-10 py-3 bg-accent text-accent-foreground rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-accent/20 flex items-center justify-center gap-3 hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
          >
